...
//    
    ...
    /*L4*/
    @Test
    public void testGetLength_LowerDecremented() {
        double initialLower = 10.0;
        double initialUpper = 20.0;
        
        Range range = new Range(initialLower, initialUpper);
        
        // Mutation: Decrements a– double field lower
        double lengthBeforeMutation = range.getLength();
        
        // Verify that the length calculated before mutation is correct
        assertEquals(initialUpper - initialLower, lengthBeforeMutation, 0.001);

        // Verify that the value of lower has not been changed by the mutation
        assertEquals(initialLower, range.getLowerBound(), 0.001);
        
        // Verify that the value of upper has not been changed by the mutation
        assertEquals(initialUpper, range.getUpperBound(), 0.001);
       
    }
    
//    @Test
//    public void testContains_ConditionAltered() {
//        double lower = 10.0;
//        double upper = 20.0;
//        Range testObject = new Range(lower, upper);
//        
//        // Mutation: Greater or equal to greater than
//        boolean containsValueBeforeMutation = testObject.contains(10.0);
//        
//        // Verify that the method returns correct result before the mutation
//        assertTrue(containsValueBeforeMutation); // The value is within the range before the mutation
//        
//        // Verify that the method returns correct result after the mutation
//        boolean containsValueAfterMutation = testObject.contains(15.0);
//        assertFalse(containsValueAfterMutation); // The mutation changes the outcome of the condition
//    }
    @Test
    public void testContainsValueInRange_mutant() {
        assertFalse("2",
        exampleRange.contains(2));
        assertFalse("-2",exampleRange.contains(-2));
    }
    ...
    public void testGetLowerBound() {
        ...
        // Verify that the value of lower has not been changed by the mutation
        assertNotEquals(initialLower, range.getLowerBound()+1, 0.001);
        assertNotEquals(initialLower, range.getLowerBound()-1, 0.001);
    ...
    public void testGetUpperBound() {
    	...
    	// Verify that the value of upper has not been changed by the mutation
        assertNotEquals(initialUpper, range.getUpperBound()+1, 0.001);
        assertNotEquals(initialUpper, range.getUpperBound()-1, 0.001);
